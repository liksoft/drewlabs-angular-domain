...
import { RessourcesWallet } from '../../auth/models/wallet';
    ...
    Object.keys(value).forEach((key) => {
      ...
      if (metadata) {
        ...
        if (isArray(designType)) {
          ...
          $serialized[entryKey] = isDefined(value[key]) ? (value[key] as Array<any>).map((k) => this.serialize(metadata.valueType, k)) : null;
        } else if (!isPrimitive(designType)) {
