...
import { createAction, DrewlabsFluxStore, StoreAction, DefaultStoreAction, onErrorAction } from '../../../rxjs/state/rx-state';
...
const deserializeSerializedUser = (serialized: any) => {
  return new GenericUndecoratedSerializaleSerializer()
    .fromSerialized(User, serialized) as User;
};
...
export const getUsersAction = (store: DrewlabsFluxStore<AppUsersState, Partial<StoreAction>>) =>
  createAction(store, (
...
export const getManageableUsersAction = (store: DrewlabsFluxStore<AppUsersState, Partial<StoreAction>>) =>
  createAction(store, (
    client: DrewlabsRessourceServerClient,
    path: string,
    params: { [index: string]: any } = {}
      ...
      payload: client.get(`${path}`, { params })
        .pipe(
          map(state => {
            const data = getResponseDataFromHttpResponse(state);
            if (isDefined(data) && isArray(data)) {
              manageableUsersDataAction(store)((data as any[]).map((current) => deserializeSerializedUser(current)));
            }
          }),
          catchError(err => {
            onErrorAction(store)(err);
...
export const manageableUsersDataAction = (store: DrewlabsFluxStore<AppUsersState, Partial<StoreAction>>) =>
  createAction(store, (payload: User[]) => {
    return {
...
export const createUserAction = (store: DrewlabsFluxStore<Partial<AppUsersState>, Partial<StoreAction>>) =>
  createAction(
...
export const userCreatedAction = (store: DrewlabsFluxStore<Partial<AppUsersState>, Partial<StoreAction>>) =>
  createAction(
    store, (payload: User) => ({
...
export const updateUserAction = (store: DrewlabsFluxStore<Partial<AppUsersState>, Partial<StoreAction>>) =>
  createAction(
...
export const userUpdatedAction = (store: DrewlabsFluxStore<Partial<AppUsersState>, Partial<StoreAction>>) =>
  createAction(
    store, (payload: User) => ({
            ...
            if (isDefined(data) && isArray(data)) {
              onPaginationDataAction(store)({
                data: (data as any[]).map(
                  (current) => deserializeSerializedUser(current)),
                total
              });
            } else {
...
export const onPaginationDataAction = (
  ...
  createAction(store, (payload: PaginationData<User>) =>
    ({ type: AppUserStoreActions.PAGINATION_DATA_ACTION, payload }));
...
export const userDeletedAction = (
  ...
  createAction(store, (payload: any) =>
    ({ type: AppUserStoreActions.DELETE_USER_ACTION, payload }));

export const resetUserStore = (store: DrewlabsFluxStore<AppUsersState, Partial<StoreAction>>) =>
  createAction(store, () => {
    return {
