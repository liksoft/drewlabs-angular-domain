...
export class SortableDirective implements AfterContentInit {
  ...
  private draggingEvent: HTMLElement;
  private dropSucceed: boolean;
  ...
  dragStart(event: any) {
    ...
    this.draggingEvent = this.getDraggableElement(event);
    this.dropSucceed = false;
    ...
    setTimeout(() => {
      this.draggingEvent.classList.add('dragging');
    });
  }
  ...
  dragEnd(event: MouseEvent) {
    ...
    this.draggingEvent.classList.remove('dragging');
  ...
  dragEnter(event: MouseEvent) {
    ...
    if (element && element.attributes) {
      const draggingIndex = this.draggingEvent.dataset.index;
      const dropIndex = element.dataset.index;

      if (draggingIndex !== dropIndex) {
        // Move dragging ghost element at its new position
        if (draggingIndex > dropIndex) {
          this.element.nativeElement.insertBefore(this.draggingEvent, element);
        } else {
          this.element.nativeElement.insertBefore(this.draggingEvent, element.nextSibling);
        }
        this.setDataAttributesToChildren();
      }
    }
